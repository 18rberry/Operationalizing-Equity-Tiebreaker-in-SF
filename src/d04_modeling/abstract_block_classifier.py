...
class AbstractBlockClassifier:
    ...
    def get_roc(self, param_arr):
        """
        returns pandas.DataFrame with 'tpr' and 'fpr' columns, each row corresponds to a point in a provided parameter array.
        """
        tpr_arr = []
        fpr_arr = []
    ...
    def get_heatmap(self, param_arr1, param_arr2=None):
        """
        returns tuple of numpy.Array with 'fpr' and 'fnr' rates respectively, each row and column corresponds to a parameter in the param array.
        """
    ...
    def get_confusion_matrix(self, params):
        ...
        '''
        if tuple(params) in self.confusion_dict:
            confusion_matrix = self.confusion_dict[tuple(params)]
